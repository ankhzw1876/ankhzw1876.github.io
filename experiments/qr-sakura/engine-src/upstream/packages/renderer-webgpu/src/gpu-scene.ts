...
export function createTreeAppearance(model: SeedModel): TreeAppearance {
  ...
  return {
    bloomDensity: 1.05 + random(seed, 10, 0, 7_000) * 0.25,
...
function trunkPoint(metrics: TreeMetrics, progress: number): Vec3 {
  return [
    metrics.trunkLean * (Math.sin(progress * Math.PI * 2.1) * 0.52 + progress * 0.65),
    metrics.trunkHeight * progress,
    metrics.trunkLean * 0.42 * Math.sin(progress * Math.PI * 1.6),
  ];
}
...
function addTrunk(segments: TreeSegment[], metrics: TreeMetrics): void {
  ...
  for (let index = 0; index < segmentCount; index++) {
    ...
    const bulge = (p: number) => 1 + Math.sin(p * Math.PI * 4.6) * 0.13 + Math.exp(-p * 12) * 0.28;
    const startBulge = bulge(startProgress);
    const endBulge = bulge(endProgress);
    pushSegment(segments, {
      depth: 0,
      end: trunkPoint(metrics, endProgress),
      endRadius: metrics.trunkRadius * (1 - endProgress * 0.88) * endBulge,
      seed: startProgress * 0.5,
      start: trunkPoint(metrics, startProgress),
      startRadius: metrics.trunkRadius * (1 - startProgress * 0.88) * startBulge,
    });
  }
}
...
/**
 * Local Sakura study: a single articulated trunk with tiered lateral limbs.
 * Unlike the upstream full canopy field, blossoms occupy only real twig tips.
 * All dimensions come from the URL's deterministic morphSeed.
 */
function createTree(model: SeedModel): {
  ...
  const seed = sceneSeed(model);
  const size = model.qrSize / 29;
  const original = measureTree(model);
  const trunkHeight = (0.46 + random(seed, 41, 0, 210) * 0.055) * size;
  const metrics: TreeMetrics = {
    ...original,
    archetype: "cloud",
    trunkHeight,
    trunkRadius: (0.027 + random(seed, 42, 0, 220) * 0.008) * size,
    trunkLean: (0.04 + random(seed, 43, 0, 230) * 0.035) * (random(seed, 43, 1, 230) < 0.5 ? -1 : 1) * size,
    canopyRadius: 0.325 * size,
    canopyDepth: 0.31 * size,
    canopyBaseY: trunkHeight * 0.42,
    canopyDomeHeight: trunkHeight * 0.7,
    minHeight: trunkHeight * 0.35,
    maxHeight: trunkHeight * 1.2,
  };
  const segments: TreeSegment[] = [];
  const tips: TreeTip[] = [];
  addTrunk(segments, metrics);
  const tiers = 5;
  const phase = random(seed, 44, 0, 240) * Math.PI * 2;
  for (let tier = 0; tier < tiers; tier++) {
    const tierProgress = tier / (tiers - 1);
    const limbs = tier === 4 ? 2 : 2 + (random(seed, tier, 0, 250) > 0.63 ? 1 : 0);
    for (let limb = 0; limb < limbs; limb++) {
      const id = tier * 7 + limb;
      const angle = phase + tier * 2.17 + (limb / limbs) * Math.PI * 2
        + (random(seed, id, 0, 260) - 0.5) * 0.55;
      const trunkT = 0.39 + tierProgress * 0.53
        + (random(seed, id, 0, 270) - 0.5) * 0.05;
      const root = trunkPoint(metrics, trunkT);
      const reach = metrics.canopyRadius * (0.88 - tierProgress * 0.27)
        * (0.84 + random(seed, id, 0, 280) * 0.2);
      let previous = root;
      let radius = metrics.trunkRadius * (0.43 - tierProgress * 0.12);
      for (let part = 0; part < 3; part++) {
        const t = (part + 1) / 3;
        const bend = (random(seed, id, part, 290) - 0.5) * 0.10;
        const end: Vec3 = [
          root[0] + Math.cos(angle + bend) * reach * t,
          root[1] + reach * (0.14 * t + 0.16 * t * t),
          root[2] + Math.sin(angle + bend) * reach * t,
        ];
        const nextRadius = radius * 0.54;
        pushSegment(segments, {
          start: previous, end, startRadius: radius, endRadius: nextRadius,
          depth: 1, seed: random(seed, id, part, 300),
        });
        if (part >= 1) {
          for (let fork = 0; fork < 2; fork++) {
            const forkAngle = angle + (fork ? 0.64 : -0.64)
              + (random(seed, id, part * 2 + fork, 310) - 0.5) * 0.4;
            const length = reach * (0.17 + random(seed, id, fork + part * 2, 320) * 0.14);
            const tip: Vec3 = [
              end[0] + Math.cos(forkAngle) * length,
              end[1] + length * (0.52 + random(seed, id, fork, 330) * 0.6),
              end[2] + Math.sin(forkAngle) * length,
            ];
            pushSegment(segments, {
              start: end, end: tip,
              startRadius: nextRadius * 0.65,
              endRadius: 0.0014 * size,
              depth: 2, seed: random(seed, id, part * 2 + fork, 340),
            });
            tips.push({position: tip, radius: 0.13 + random(seed, id, fork + part * 2, 350) * 0.06});
          }
        }
        previous = end;
        radius = nextRadius;
      }
      tips.push({ position: previous, radius: 0.17 + random(seed, id, 0, 360) * 0.06 });
    }
  }
  // A small split at the crown keeps an airy, tree-like apex.
  const crown = trunkPoint(metrics, 0.96);
  for (let arm = 0; arm < 3; arm++) {
    const angle = phase + arm * 2.0944;
    const end: Vec3 = [
      crown[0] + Math.cos(angle) * 0.09 * size,
      crown[1] + (0.055 + random(seed, arm, 0, 370) * 0.035) * size,
      crown[2] + Math.sin(angle) * 0.09 * size,
    ];
    pushSegment(segments, {
      start: crown, end, startRadius: 0.007 * size, endRadius: 0.0018 * size,
      depth: 2, seed: random(seed, arm, 0, 380),
    });
    tips.push({position: end, radius: 0.16});
  }
...
function addTipCluster(
  ...
  const count = 18 + Math.floor(12 * appearance.bloomDensity * density);
  for (let index = 0; index < count; index++) {
    ...
    const y = tip.position[1] + radius * vertical * 0.38 + clusterRadius * 0.1;
  ...
  const hangingCount = 3 + Math.floor(3 * random(seed, tipIndex, 0, 1_500));
...
function createGroundPetals(model: SeedModel, appearance: TreeAppearance): Float32Array {
  ...
  const count = Math.round(25 * (0.82 + appearance.bloomDensity * 0.18));
  for (let index = 0; index < count; index++) {
...
function createFlowers(
  ...
  // No spherical canopy overlay: preserve the open spaces between tiered boughs.
  tips.forEach((tip, index) => addTipCluster(flowers, model, appearance, tip, index));
  // The reference's meadow has small pink flowers at the ends of tall grass.
  const center = model.qrSize / 2;
  for (const module of model.modules) {
    const column = module.index % model.qrSize;
    const row = Math.floor(module.index / model.qrSize);
    if ((column - center) ** 2 + (row - center) ** 2 < (model.qrSize * 0.46) ** 2) continue;
    for (let index = 0; index < 3; index++) {
      const seed = random(model.morphSeed, module.index, index, 12_000);
      flowers.push(
        column + 0.15 + random(model.morphSeed, module.index, index, 12_100) * 0.7,
        row + 0.15 + random(model.morphSeed, module.index, index, 12_200) * 0.7,
        SEED_BLOCK_SIZE * (1.9 + seed * 1.5),
        seed,
      );
    }
  }
...
function createGrass(model: SeedModel): Float32Array {
  ...
  for (const module of model.modules) {
    ...
    const bladeCount = 16 + Math.floor(8 * random(model.morphSeed, column, row, 6_100));
    for (let blade = 0; blade < bladeCount; blade++) {
      ...
      const height = 1.1 + 1.7 * random(model.morphSeed, column, row, 6_500 + blade);
