...
- XiaHua OS presents the warm-spring sakura as its own top-level, closable window instead of a fixed object beside the homepage statement or a desktop shortcut. On a normal launch the window is prepared behind the outgoing boot cover, so the desktop reveal contains one continuous warm-spring scene rather than a second popup. Deep links still take priority. The window launches with `embed=1&palette=spring&world=sakura&target=wechat`; the whitelisted target sends `http://weixin.qq.com/r/mp/9RM9JQPEX14grUdG90bn` directly into the native tree and QR generator. No supplied QR bitmap is layered over the canvas. It pauses when minimized or covered, unloads when closed and can be reopened from the system menu. The ordinary Playground entry remains a free-URL, four-world experiment.
