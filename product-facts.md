...
- XiaHua OS presents the warm-spring sakura as its own top-level, openable and closable window instead of a fixed object beside the homepage statement. It lazily hydrates only while that window is foreground, launches with the internal `embed=1&palette=spring&world=sakura&qr=profile` preset, pauses when minimized or covered, and unloads when closed. The whitelisted profile preset binds both the generated tree model and the final supplied 430×430 QR image to the decoded WeChat target `http://weixin.qq.com/r/mp/9RM9JQPEX14grUdG90bn`; arbitrary QR image paths are not accepted. Chromium `BarcodeDetector` decoded the original supplied file before integration. The ordinary Playground entry remains a free-URL, four-world experiment and does not activate the exact-image handoff.
